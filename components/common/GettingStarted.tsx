  ...
  return (
    ...
    <div className="absolute h-[27rem] w-2/3 rounded-xl flex justify-center items-center bg-[#141414] left-[52.5%] border border-[#242424] -mt-14 top-0 transform -translate-x-1/2 translate-y-1/2">
      <div className="flex flex-col pt-16 justify-center items-center h-[92.7%] w-[96.7%] mt-0.5 bg-[#1a1b1b] rounded-xl border border-[#242424]">
        <h1 className="text-white text-3xl font-semibold tracking-wide">{storedUser?.stepsCompleted || 0 >= 1 ? "Welcome back 👋" : "Hey there 👋"}, let's get started:</h1>
        <ol className="mt-6 list-decimal flex flex-col items-center">
          {stepsList.map((step) => (
            <li key={step.stepValue} className={step.isactive ? activeClasses : step.isCompleted ? completedClasses : inactiveClasses}>{step.label}</li>
          ))}
        </ol>
        ...
        {(storedUser?.stepsCompleted || 0) === 1 && <a href={`https://github.com/apps/gitdocs-ai/installations/new`} target="_blank" rel="noopener noreferrer" onClick={() => handleAddRepository()} className="bg-[#0078d4] hover:bg-[#0078d4]/90 mt-8 text-white px-8 py-1.5 rounded-md">Configure Github</a>}

        {(storedUser?.stepsCompleted || 0) === 2 && <button className="bg-[#0078d4] hover:bg-[#0078d4]/90 mt-8 text-white px-8 py-1.5 rounded-md" onClick={() => handleViewDashboard()}>View Dashboard</button>}

        <div className="flex justify-center gap-8 items-center mt-14">
          {buttonItems.map((item) => (
            item.show && (
              <a key={item.label} href={item.href} target="_blank" className={`text-[0.9rem] border-[#383737] flex gap-2 items-center hover:text-white hover:bg-[#282829] transition-all duration-100 px-5 py-2 rounded-md -ms-1`}>
                <item.icon size={16} />
